...
let path='', items=[], writable=false, sortKey='name', sortAsc=true;
...
async function load(p=path){try{let d=await api('/api/list?path='+encodeURIComponent(p));path=d.path;items=d.items;writable=d.writable;$('#title').textContent=d.title;$('#status').textContent=writable?t('readWrite'):t('readOnly');document.querySelectorAll('.write').forEach(x=>x.hidden=!writable);crumbs();render()}catch(e){toast(e.message,true)}}
